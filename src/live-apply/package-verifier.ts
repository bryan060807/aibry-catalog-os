...
import type { LyricSourceApplyHandoff, LyricSourceDryRunReport, LyricSourceOperatorPackage } from "../lyric-source/contracts.js";
...
export async function verifyGuardedOperatorPackage(manifestInput: string): Promise<GuardedPackageVerification> {
  ...
  if (
    ...
    handoff.operations.length !== 7 || canonicalJson(handoff.operations) !== canonicalJson(proposal.operations) ||
  ...
  if ([expectedBaselineCounts.catalogFindings, expectedBaselineCounts.assetFindings, expectedPostApplyCounts.catalogFindings, expectedPostApplyCounts.assetFindings].some((value) => !Number.isInteger(value) || value < 0)) {
