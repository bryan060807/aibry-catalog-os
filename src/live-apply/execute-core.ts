...
async function assertSuccessfulResult(plan: GuardedLiveApplyPlan, verifiedPackage: GuardedPackageVerification, result: Record<string, unknown>, report: GuardedLiveApplyLaunchReport): Promise<void> {
  if (result.contract !== "lyric-source-apply-result.v1" || result.status !== "applied-and-validated" || result.proposalId !== plan.proposalId || result.proposalCanonicalSha256 !== plan.proposalSha256 || result.operationCount !== verifiedPackage.operations.length) throw new Error("Generated APPLY result contract, status, proposal, or operation count is invalid.");
...
async function verifyRollback(packageRoot: string, verifiedPackage: GuardedPackageVerification, restoredVaultRoot?: string, expectedRollbackRoot?: string): Promise<boolean> {
  try {
    ...
    if (manifest.contract !== "lyric-source-rollback-manifest.v1" || manifest.targetCount !== verifiedPackage.operations.length || manifest.targets.length !== verifiedPackage.operations.length) return false;
